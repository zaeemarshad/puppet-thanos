# @summary This class manages query-frontned service
#
# This class install Query-Frontend as service query-frontned node which implements a service that can be put in front of Thanos Queriers to improve the read pathimplements a service that can be put in front of Thanos Queriers to improve the read path.
#
# @param ensure
...
# @query_range_max_query_parallelism
...
class thanos::query_frontend (
  ...
  Integer                        $query_range_max_query_parallelism                 = 14,
  String                         $query_range_response_cache_max_freshness          = '1m',
  String                         $query_frontend_downstream_url                     = 'http://localhost:9091',
  Integer                        $query_frontend_log_queries_longer_than            = 0,
  Enum['LogFinishCall', 'LogStartAndFinishCall', 'NoLogCall'] $log_request_decision = 'LogFinishCall',
  Hash                           $extra_params                                      = {},
) {
  ...
  thanos::resources::service { 'query-frontend':
    ...
    params       => {
      ...
      'query-range.max-query-parallelism'        => $query_range_max_query_parallelism,
